...
const styles = `
  ...
  * { box-sizing: border-box; }
  ...
  .progress-track { background: #E5E5EA; height: 8px; border-radius: 999px; overflow: hidden; }
  .progress-fill { background: var(--accent); height: 100%; transition: width 0.5s cubic-bezier(0.4, 0, 0.2, 1); border-radius: 999px; }
...
// --- CONFIGURACIÓN DE FIREBASE ADAPTADA PARA CANVAS ---
...
export default function App() {
  ...
  const toggleEventCompletion = async (eventObj, e) => {
    if (e) e.stopPropagation();
    if (user) {
      try {
        const eventRef = doc(collection(db, 'artifacts', appId, 'users', user.uid, 'events'), eventObj.id);
        await updateDoc(eventRef, { completed: !eventObj.completed });
        if (selectedEvent && selectedEvent.id === eventObj.id) {
          setSelectedEvent({ ...selectedEvent, completed: !eventObj.completed });
        }
      } catch (error) { console.error(error); }
    }
  };
        ...
        <div key={`prev-${i}`} className="day-cell faded" onClick={() => handleDayClick(y, m, dayNum)}>
          ...
          <div className="events-container">
            {dayEvents.sort((a, b) => {
              ...
              <div key={event.id} className={`event-pill ${event.color} ${event.completed ? 'completed' : ''} animate-pop`} onClick={(e) => openViewModal(e, event)}>
          ...
          <div className="sidebar-list">
            <div className="progress-card">
              <div className="progress-header">
                <h4 className="progress-title">Progreso Semanal</h4>
                <span className="progress-stats">{weeklyStats.completed}/{weeklyStats.total}</span>
              </div>
              <div className="progress-track">
                <div className="progress-fill" style={{ width: `${weeklyStats.percentage}%` }}></div>
              </div>
            </div>
            ...
            <div className="events-list-container">
              {currentMonthEventsList.length === 0 ? (
                ...
                currentMonthEventsList.map(event => {
                  ...
                  return (
                    <div key={`list-${event.id}`} className={`list-item ${event.color} ${event.completed ? 'completed' : ''}`} onClick={(e) => openViewModal(e, event)}>
          ...
          <div className="modal-overlay" onClick={closeViewModal}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <div className="view-event-header">
                ...
                <div style={{ flex: 1 }}>
                  <h4 className="view-event-title" style={{ textDecoration: selectedEvent.completed ? 'line-through' : 'none', color: selectedEvent.completed ? 'var(--text-muted)' : 'var(--text-main)' }}>{selectedEvent.title}</h4>
                ...
                <div className={`check-circle ${selectedEvent.completed ? 'checked' : ''}`} style={{ width: 36, height: 36 }} onClick={(e) => toggleEventCompletion(selectedEvent, e)}>
                  {selectedEvent.completed && <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                </div>
              </div>
